...
class TestCluster(unittest.TestCase):
    ...
    def test_smear(self):
        ...
        for i in range(10000):
            smeared = simulator.make_and_store_smeared_cluster(cluster, ecal, accept=True)
    ...
    def test_acceptance(self):
        ...
        h_ptvseta = TH2F('h_ptvseta','cluster pt vs eta',
                         100, -5, 5, 100, 0, 15)
        nclust = 1000 #otherwise alice gets numpy error message to say 'float' object cannot be interpreted as an index
        ...
        for cluster in clusters:
            smeared_cluster = simulator.make_and_store_smeared_cluster(cluster, ecal)
            if smeared_cluster:
