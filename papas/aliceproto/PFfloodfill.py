...
class Block(object):
    ...
    def dosort(self):
        self.linked.sort(key=self.getType)
...
class FloodFill(object):
    ...
    def visit(self, element):
        ...
        if self.visited.get(element, False):
            ...
            self.groups.setdefault(element.block_label, Block(element.block_label)).append(element)
    ...
    def __str__(self):
        ...
        for gid, group in self.groups.iteritems():
            groupinfo = ', '.join(map(str, group.linked))
